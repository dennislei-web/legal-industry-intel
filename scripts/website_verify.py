...
def fetch_homepage(homepage, timeout=15):
    """抓首頁 HTML；死站/4xx/5xx 回 None。verify=False：憑證壞掉的所仍算有官網"""
    try:
        resp = requests.get(homepage, headers=VERIFY_HEADERS, timeout=timeout,
                            verify=False, allow_redirects=True)
        if resp.status_code >= 400 or not resp.text:
            return None, None
        if resp.encoding in (None, 'ISO-8859-1'):
            resp.encoding = resp.apparent_encoding or 'utf-8'
        final_home = homepage_of(resp.url) or homepage
        return resp.text, final_home
    except Exception:
        return None, None
