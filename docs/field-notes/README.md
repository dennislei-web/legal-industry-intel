...
- 首筆拜訪的寫法可參考 migration 196（資料 migration 只存本機，不在 repo）
...
| 欄位 | 規則 |
|---|---|
| `firm`（notes） | 受訪者所屬所，用 `moj_firm_statistics().firm_name` 的全名（例「○○法律事務所」），前端靠這個名字對到事務所 modal；**先查 DB 對齊異體字**（異體字、「國際」字樣、英文名對中文名等），所名未載時寫「（未載）○○所屬事務所」，**佔位名不得含人名** |
