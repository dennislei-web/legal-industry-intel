-- 185: 幽靈院規則 v2（放寬稀疏散佈＋名冊豁免；接續 mig 184）
--
-- 184 之後的殘留稽核：仍有 137 個「法官×法院」列是幽靈院當錨（該院不在名冊、常態院才是名冊
-- 現任院），合計 1,384 件——成因是 184 要求月數 ≥6，抓不到「3 個月各 1 件卻橫跨兩年」的稀疏
-- 散佈型誤抓。放寬為（≥6 月）OR（≥2 月且區間跨度 >6 月）後多抓 348 列／1,413 件。
--
-- 但放寬會誤殺 6 位最高行政法院現任法官（該院裁判書公開量天生極少，現任者常只有 2-3 個月
-- 各 1 件署名，型態與誤抓無法用密度區分）。故加「名冊豁免」：司法院名冊有該人於該院者
-- 一律不判幽靈。加上豁免後誤殺歸零。前端 index.html 以 judges_combined.has_jy 實作同一條件。
...
BEGIN
  ...
  -- a2) 幽靈院：（≥6 月，或 ≥2 月且區間跨度 >6 月）、月均 ≤3 件、<本人常態院的 10%、
  --     審級高於常態院、且非司法院名冊現任院（名冊＝權威，一律豁免）
  CREATE TEMP TABLE _jghost ON COMMIT DROP AS
  WITH d AS (
    SELECT name, court_name, months, last_i - first_i + 1 AS range_mo,
    ...
    -- 常態院＝最高密度院（同密度時取月數多者，確保與前端排序結果一致）
  ...
  WHERE (d.months >= 6 OR (d.months >= 2 AND d.range_mo > 6))
    AND d.dens <= 3
    AND d.dens < t.top_dens * 0.1
    AND d.tier > t.top_tier
    AND NOT EXISTS (
      SELECT 1 FROM jy_judges j
      WHERE j.name = d.name AND normalize_court_name(j.court_name) = d.court_name);
