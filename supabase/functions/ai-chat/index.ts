...
async function buildUserContext(userClient: SupabaseClient, serviceClient: SupabaseClient) {
  ...
  try {
    ...
    if (topFirms?.length) {
      ...
      for (const f of topFirms.slice(0, 10)) parts.push(`  * ${f.firm_name}: ${f.lawyer_count} 位律師 (${f.main_region || '-'})`);
    }
    if (regions?.length) {
      parts.push('- 律師地區分布:');
      for (const r of regions.slice(0, 10)) parts.push(`  * ${r.region}: ${r.count} 位`);
    }
